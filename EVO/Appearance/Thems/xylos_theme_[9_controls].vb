...
Friend Module Helpers_
    ...
    Public Function RoundRect_x(ByVal Rect As Rectangle, ByVal Rounding As Integer, Optional ByVal Style As RoundingStyle = RoundingStyle.All) As Drawing2D.GraphicsPath
    ...
    Protected Overrides Sub OnPaint(ByVal e As PaintEventArgs)
        ...
        If Enabled Then
            ...
            Using Background As New SolidBrush(ColorFromHex("#F3F4F7")), Border As New Pen(ColorFromHex("#D0D5D9")), TextColor As New SolidBrush(ColorFromHex("#7C858E")), TextFont As New Font("Segoe UI", 9)
                G.FillPath(Background, RoundRect_x(New Rectangle(0, 0, 16, 16), 3))
                G.DrawPath(Border, RoundRect_x(New Rectangle(0, 0, 16, 16), 3))
...
Public Class XylosNotice
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        Using Background As New SolidBrush(ColorFromHex("#FFFDE8")), MainBorder As New Pen(ColorFromHex("#F2F3F7")), TextColor As New SolidBrush(ColorFromHex("#B9B595")), TextFont As New Font("Segoe UI", 9)
            G.FillPath(Background, RoundRect_x(FullRectangle(Size, True), 3))
            G.DrawPath(MainBorder, RoundRect_x(FullRectangle(Size, True), 3))
        ...
        If Enabled Then
            ...
            If State = MouseState.Down Then
                ...
                Using Border As New Pen(ColorFromHex("#78B7E6"))
                    G.DrawPath(Border, RoundRect_x(FullRectangle(Size, True), 12))
                End Using
...
#Region " Drawing "
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        Using Border As New Pen(ColorFromHex("#D0D5D9"))
            G.DrawPath(Border, RoundRect_x(FullRectangle(Size, True), 6))
        End Using

        If Not Value = 0 Then

            Using Background As New Drawing2D.HatchBrush(Drawing2D.HatchStyle.LightUpwardDiagonal, Stripes, BackgroundColor)
                G.FillPath(Background, RoundRect_x(New Rectangle(0, 0, Value / Maximum * Width - 1, Height - 1), 6))
            End Using
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        If Enabled Then
            ...
            Using ButtonFont As New Font("Segoe UI", 9), Border As New Pen(ColorFromHex("#C3C3C3"))
                G.DrawPath(Border, RoundRect_x(FullRectangle(Size, True), 3))
